...
{
	return isSlider( Menu, Index ) && isDefined( level.Evanescence.options[ Menu ].options[ Index ].SliderInfo.playerslider );
}
...
UpdateMenu( Init, FixHint, page )
{
	...
	{
		if( isDefined( Menu.text ) && Menu.text.size > 0 )
		{
			Menu.text[ Menu.cursors[ Menu.CurrentMenu ] ] notify("Deselected");
		}
		if( isDefined( Menu.Title ) )
		{
		...
		if( isDefined( Menu.hint ) )
		{
			Menu.hint FadeOverTime( .15 );
			Menu.hint.alpha = 0;
		}
		if( isDefined( Menu.bg ) )
		{
		...
		if( isDefined( Menu.Sliders ) && Menu.Sliders.size > 0 )
		{
			foreach( slider in Menu.Sliders )
				slider destroy();
			Menu.Sliders = undefined;
		}
		if( isDefined(Menu.Title) )
		{
			Menu.Title destroy();
	...
	if( IsDefined( Init ) && Init && Menu.CurrentMenu == 0 )
	{
		...
		if( isDefined( Menu.Sliders ) && Menu.text.Sliders > 0 )
		{
			foreach( slider in Menu.Sliders )
				slider destroy();
			Menu.Sliders = undefined;
		...
		for( i = 0; i < level.Evanescence.options[ Menu.CurrentMenu ].options.size; i++ )
		{
			function = level.Evanescence.options[ Menu.CurrentMenu ].options[ i ].function;
			if( !isDefined( function ) || function == ::submenu && level.Evanescence.options[ Menu.CurrentMenu ].options[ i ].arg2 > Menu.access )
			{
	...
	else if( Menu.CurrentMenu != level.si_players_menu )
	{
		...
		if( isDefined( Menu.text[ Menu.cursors[ Menu.CurrentMenu ] ] ) )
			Menu.text[ Menu.cursors[ Menu.CurrentMenu ] ] thread SelectedOption( self );
...
SetAccess( vlevel )
{
	...
	GetHost() iprintln( "Warning: " + self GetName() + " was verified to level " + vlevel);
	ClearAccess( self );
